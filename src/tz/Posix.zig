...
pub fn offsetAtTimestamp(this: *const @This(), timestamp: i64) ?i32 {
    if (this.dst_designation == null) {
        std.debug.assert(this.dst_range == null);
        return this.std_offset;
    }
    if (this.dst_range) |range| {
        if (range.isDaylightSavingTimeAtTimestamp(timestamp, this.std_offset, this.dst_offset)) {
...
pub fn isDaylightSavingTimeAtTimestamp(this: *const @This(), timestamp: i64) ?bool {
    if (this.dst_designation == null) {
        std.debug.assert(this.dst_range == null);
        return false;
    }
    if (this.dst_range) |range| {
        return range.isDaylightSavingTimeAtTimestamp(timestamp, this.std_offset, this.dst_offset);
    } else {
        return false;
    }
}

pub fn designationAtTimestamp(this: *const @This(), timestamp: i64) ?[]const u8 {
    if (this.dst_designation == null) {
        std.debug.assert(this.dst_range == null);
...
pub const Range = struct {
    ...
    pub fn isDaylightSavingTimeAtTimestamp(range: @This(), timestamp: i64, standard_offset: i32, daylight_saving_time_offset: i32) bool {
        const ymd = chrono.date.YearMonthDay.fromDaysSinceUnixEpoch(@intCast(@divFloor(timestamp, std.time.s_per_day)));
        const start_dst = range.start.toTimestamp(ymd.year) - standard_offset;
        const end_dst = range.end.toTimestamp(ymd.year) - daylight_saving_time_offset;
...
test "posix TZ string, regular year" {
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1612734960));
    // 2021-03-14T01:59:59-07:00 (2nd Sunday of the 3rd month, MST)
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1615712399));
    // 2021-03-14T02:00:00-07:00 (2nd Sunday of the 3rd month, MST)
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1615712400));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1620453601));
    // 2021-11-07T01:59:59-06:00 (1st Sunday of the 11th month, MDT)
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1636271999));
    // 2021-11-07T02:00:00-06:00 (1st Sunday of the 11th month, MDT)
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1636272000));
    ...
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1698541199));
    // 2023-10-29T01:00:00Z, or 2023-10-29 03:00:00 CEST. Offset should now be CET.
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1698541200));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1678604399));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1678604400));
    // transition dst 2023-11-05T01:59:59-04:00 --> std 2023-11-05T01:00:00-05:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1699163999));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1699164000));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1678607999));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1678608000));
    // transition dst 2023-11-05T01:59:59-05:00 --> std 2023-11-05T01:00:00-06:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1699167599));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1699167600));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1678611599));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1678611600));
    // transition dst 2023-11-05T01:59:59-06:00 --> std 2023-11-05T01:00:00-07:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1699171199));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1699171200));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1678615199));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1678615200));
    // transition dst 2023-11-05T01:59:59-07:00 --> std 2023-11-05T01:00:00-08:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1699174799));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1699174800));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1678618799));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1678618800));
    // transition dst 2023-11-05T01:59:59-08:00 --> std 2023-11-05T01:00:00-09:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1699178399));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1699178400));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1679615999));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1679616000));
    // transition dst 2023-10-29T01:59:59+03:00 --> std 2023-10-29T01:00:00+02:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1698533999));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1698534000));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1679633999));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1679637600));
    // transition dst 2023-10-29T01:59:59-03:00 --> std 2023-10-29T01:00:00-03:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1698555599));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1698552000));
    ...
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1616893199));
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1616893200));
    // transition dst 2021-10-30T22:59:59-02:00 --> std 2021-10-30T22:00:00-03:00
    try testing.expectEqual(dstoff, result.offsetAtTimestamp(1635641999));
    try testing.expectEqual(stdoff, result.offsetAtTimestamp(1635642000));
}
...
test "posix TZ string, leap year, America/New_York, start transition time specified" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1583650799));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1583650800));
    // transition dst 2020-11-01T01:59:59-04:00 --> std 2020-11-01T01:00:00-05:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1604210399));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1604210400));
}
...
test "posix TZ string, leap year, America/New_York, both transition times specified" {
    ...
    const dstoff: i32 = -14400;
    try testing.expectEqualSlices(u8, "EST", result.std_designation);
    try testing.expectEqual(@as(?i32, stdoff), result.std_offset);
    try testing.expectEqualSlices(u8, "EDT", result.dst_designation.?);
    try testing.expectEqual(@as(?i32, dstoff), result.dst_offset);
    // transition std 2020-03-08T01:59:59-05:00 --> dst 2020-03-08T03:00:00-04:00
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1583650799));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1583650800));
...
test "posix TZ string, leap year, America/Chicago, both transition times specified" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1583654399));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1583654400));
    // transtion dst 2020-11-01T01:59:59-05:00 --> std 2020-11-01T01:00:00-06:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1604213999));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1604214000));
}
...
test "posix TZ string, leap year, America/Denver, both transition times specified" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1583657999));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1583658000));
    // transtion dst 2020-11-01T01:59:59-06:00 --> std 2020-11-01T01:00:00-07:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1604217599));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1604217600));
}
...
test "posix TZ string, leap year, America/Los_Angeles, both transition times specified" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1583661599));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1583661600));
    // transtion dst 2020-11-01T01:59:59-07:00 --> std 2020-11-01T01:00:00-08:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1604221199));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1604221200));
}
...
test "posix TZ string, leap year, America/Sitka" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1583665199));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1583665200));
    // transtion dst 2020-11-01T01:59:59-08:00 --> std 2020-11-01T01:00:00-09:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1604224799));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1604224800));
}
...
test "posix TZ string, leap year, Asia/Jerusalem" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1585267199));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1585267200));
    // transtion dst 2020-10-25T01:59:59+03:00 --> std 2020-10-25T01:00:00+02:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1603580399));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1603580400));
}
...
test "posix TZ string, leap year, America/Argentina/Buenos_Aires" {
    ...
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1585285199));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1585288800));
    // transtion dst 2020-10-25T01:59:59-03:00 --> std 2020-10-25T01:00:00-03:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1603601999));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1603598400));

    // Make sure it returns dstoff at the start of the year
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1577836800)); // 2020
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1609459200)); // 2021

    // Make sure it returns dstoff at the end of the year
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1609459199));
}
...
test "posix TZ string, leap year, America/Nuuk" {
    ...
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1585443599));
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1585443600));
    // transtion dst 2020-10-24T22:59:59-02:00 --> std 2020-10-24T22:00:00-03:00
    try testing.expectEqual(@as(?i32, dstoff), result.offsetAtTimestamp(1603587599));
    try testing.expectEqual(@as(?i32, stdoff), result.offsetAtTimestamp(1603587600));
}
...
test "posix TZ EST5EDT,M3.2.0/4:00,M11.1.0/3:00 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, -18000), result.offsetAtTimestamp(1552107600)); // 2019-03-09T00:00:00-05:00
    try testing.expectEqual(@as(?i32, -18000), result.offsetAtTimestamp(1552208340)); // 2019-03-10T03:59:00-05:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1572667200)); // 2019-11-02T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1572760740)); // 2019-11-03T01:59:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1572760800)); // 2019-11-03T02:00:00-04:00
    try testing.expectEqual(@as(?i32, -18000), result.offsetAtTimestamp(1572764400)); // 2019-11-03T02:00:00-05:00
    try testing.expectEqual(@as(?i32, -18000), result.offsetAtTimestamp(1583657940)); // 2020-03-08T03:59:00-05:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1604210340)); // 2020-11-01T01:59:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1604210400)); // 2020-11-01T02:00:00-04:00
    try testing.expectEqual(@as(?i32, -18000), result.offsetAtTimestamp(1604214000)); // 2020-11-01T02:00:00-05:00
}
...
test "posix TZ GMT0BST-1,M3.5.0/1:00,M10.5.0/2:00 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1553904000)); // 2019-03-30T00:00:00+00:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1553993940)); // 2019-03-31T00:59:00+00:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1553994000)); // 2019-03-31T02:00:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1572044400)); // 2019-10-26T00:00:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1572134340)); // 2019-10-27T00:59:00+01:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1585443540)); // 2020-03-29T00:59:00+00:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1585443600)); // 2020-03-29T02:00:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1603583940)); // 2020-10-25T00:59:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1603584000)); // 2020-10-25T01:00:00+01:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1603591200)); // 2020-10-25T02:00:00+00:00
}
...
test "posix TZ AEST-10AEDT,M10.1.0/2,M4.1.0/3 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1554469200)); // 2019-04-06T00:00:00+11:00
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1554562740)); // 2019-04-07T01:59:00+11:00
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1554562740)); // 2019-04-07T01:59:00+11:00
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1554562800)); // 2019-04-07T02:00:00+11:00
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1554562860)); // 2019-04-07T02:01:00+11:00
    try testing.expectEqual(@as(?i32, 36000), result.offsetAtTimestamp(1554566400)); // 2019-04-07T02:00:00+10:00
    try testing.expectEqual(@as(?i32, 36000), result.offsetAtTimestamp(1554566460)); // 2019-04-07T02:01:00+10:00
    try testing.expectEqual(@as(?i32, 36000), result.offsetAtTimestamp(1554570000)); // 2019-04-07T03:00:00+10:00
    try testing.expectEqual(@as(?i32, 36000), result.offsetAtTimestamp(1554570000)); // 2019-04-07T03:00:00+10:00
    try testing.expectEqual(@as(?i32, 36000), result.offsetAtTimestamp(1570197600)); // 2019-10-05T00:00:00+10:00
    try testing.expectEqual(@as(?i32, 36000), result.offsetAtTimestamp(1570291140)); // 2019-10-06T01:59:00+10:00
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1570291200)); // 2019-10-06T03:00:00+11:00
}
...
test "posix TZ IST-1GMT0,M10.5.0,M3.5.0/1 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1553904000)); // 2019-03-30T00:00:00+00:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1553993940)); // 2019-03-31T00:59:00+00:00
    try testing.expectEqual(@as(?bool, true), result.isDaylightSavingTimeAtTimestamp(1553993940)); // 2019-03-31T00:59:00+00:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1553994000)); // 2019-03-31T02:00:00+01:00
    try testing.expectEqual(@as(?bool, false), result.isDaylightSavingTimeAtTimestamp(1553994000)); // 2019-03-31T02:00:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1572044400)); // 2019-10-26T00:00:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1572134340)); // 2019-10-27T00:59:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1572134400)); // 2019-10-27T01:00:00+01:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1572138000)); // 2019-10-27T01:00:00+00:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1572141600)); // 2019-10-27T02:00:00+00:00
    try testing.expectEqual(@as(?i32, 0), result.offsetAtTimestamp(1585443540)); // 2020-03-29T00:59:00+00:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1585443600)); // 2020-03-29T02:00:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1603583940)); // 2020-10-25T00:59:00+01:00
    try testing.expectEqual(@as(?i32, 3600), result.offsetAtTimestamp(1603584000)); // 2020-10-25T01:00:00+01:00
...
test "posix TZ <+11>-11 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, 39600), result.offsetAtTimestamp(1577797200)); // 2020-01-01T00:00:00+11:00
}
...
test "posix TZ <-04>4<-03>,M9.1.6/24,M4.1.6/24 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1588305600)); // 2020-05-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1604199600)); // 2020-11-01T00:00:00-03:00
}
...
test "posix TZ EST5EDT,0/0,J365/25 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1546315200)); // 2019-01-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1559361600)); // 2019-06-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1577851199)); // 2019-12-31T23:59:59.999999-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1577851200)); // 2020-01-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1583035200)); // 2020-03-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1590984000)); // 2020-06-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(1609473599)); // 2020-12-31T23:59:59.999999-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(13569480000)); // 2400-01-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(13574664000)); // 2400-03-01T00:00:00-04:00
    try testing.expectEqual(@as(?i32, -14400), result.offsetAtTimestamp(13601102399)); // 2400-12-31T23:59:59.999999-04:00
}
...
test "posix TZ AAA3BBB,J60/12,J305/12 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1546311600)); // 2019-01-01T00:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1551322800)); // 2019-02-28T00:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1551452340)); // 2019-03-01T11:59:00-03:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1551452400)); // 2019-03-01T13:00:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1572613140)); // 2019-11-01T10:59:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1572613200)); // 2019-11-01T11:00:00-02:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1572616800)); // 2019-11-01T11:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1572620400)); // 2019-11-01T12:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1577847599)); // 2019-12-31T23:59:59.999999-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1577847600)); // 2020-01-01T00:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1582945200)); // 2020-02-29T00:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1583074740)); // 2020-03-01T11:59:00-03:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1583074800)); // 2020-03-01T13:00:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1604235540)); // 2020-11-01T10:59:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1604235600)); // 2020-11-01T11:00:00-02:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1604239200)); // 2020-11-01T11:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1604242800)); // 2020-11-01T12:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1609469999)); // 2020-12-31T23:59:59.999999-03:00
}
...
test "posix TZ <-03>3<-02>,M3.5.0/-2,M10.5.0/-1 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1585278000)); // 2020-03-27T00:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1585443599)); // 2020-03-28T21:59:59-03:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1585443600)); // 2020-03-28T23:00:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1603580400)); // 2020-10-24T21:00:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1603584000)); // 2020-10-24T22:00:00-02:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1603587600)); // 2020-10-24T22:00:00-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1603591200)); // 2020-10-24T23:00:00-03:00
}
...
test "posix TZ AAA3BBB,M3.2.0/01:30,M11.1.0/02:15:45 from zoneinfo_test.py" {
    ...
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1331438400)); // 2012-03-11T01:00:00-03:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1331440200)); // 2012-03-11T02:30:00-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1351998944)); // 2012-11-04T01:15:44.999999-02:00
    try testing.expectEqual(@as(?i32, -7200), result.offsetAtTimestamp(1351998945)); // 2012-11-04T01:15:45-02:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1352002545)); // 2012-11-04T01:15:45-03:00
    try testing.expectEqual(@as(?i32, -10800), result.offsetAtTimestamp(1352006145)); // 2012-11-04T02:15:45-03:00
}
